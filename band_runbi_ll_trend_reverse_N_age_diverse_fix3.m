...
function [b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_trend_reverse_N_age_diverse_fix3(X, trend_hat, band_age, topics_count, X_N, None0s_X_N, S, y, beta_0, beta_fix)
...
options = optimset('Display','iter','LargeScale','on','GradObj','on','Hessian','off','TolFun',1e-6, 'TolX',1e-14, 'MaxIter',1e3, 'MaxFunEvals', 1e5, 'PlotFcns',@optimplotfirstorderopt);  % ,'OutputFcn', @showJ_history);   %, 'FinDiffType', 'central');
...
[b, fval,exitflag,output,grad,hessian] = fminunc(@band_bi_ll_trend_reverse_N_age_diverse_fix3,beta0,options,IVs, trend_hat, band_age, topics_count, X_N, None0s_X_N, S, choice_dv, beta_fix);
...
assert(det(hessian)==0, 'Hessian is singular');
