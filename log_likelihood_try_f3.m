function [LL, g, H] = log_likelihood_try_f3(b,DV,IV,I,P)

b_basics = b(1:end-1);

% exp_util = exp(-[IV(:,1:4) normpdf(IV(:,5),0,exp(b(end)))]*b_basics');         % this is now the utility of the external good
exp_util = exp(-[IV(:,1:4) gampdf(IV(:,5),1,exp(b(end)))]*b_basics');
...
g(5) = gampdf(IV(:,5),1,exp(b(end)))'*(prob-DV(:,1));
% normal_derivative_wrt_sigmas = b(end-1).*2^(-5/2).*(IV(:,5).^2-1).*normpdf(IV(:,5),0,exp(b(end)))./exp(b(end));
normal_derivative_wrt_sigmas = gamma_derivative_wrt_beta(IV(:,5), 1, b(end)); 
