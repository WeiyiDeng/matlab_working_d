...
interarrival_day = round(exprnd(8))               % w: 为第一个火苗设定随机发生时间
...
for d =1:200   
    cla                                           % clean up the track of move
     ...
     %if d == df+10
           ...
           interarrival_day = round(exprnd(8))               % w: 下一个火苗的发生时间不能d每循环一次都变，而应该是一个相对固定值只在每次新着火点产生后更新
           if i > 1
               for j = 1:i
     ...
     'FaceColor','r') 
      ...
      for k=1:i
      x(k)=x(k)+(-1+2*rand)*5 %x点随机移动
      y(k)=y(k)+(-1+2*rand)*5 %x点随机移动
      r(k)=r(k)+0.4
