...
band_pop_sum_past2_year = sparse(max(band_pop(:,3)),max(band_pop(:,4)));
for t = 1:103
    band_pop_sum_past2_year(:,t) = -1;
end
for t = 104:size(band_pop_sum_past2_year,2)
    band_pop_sum_past2_year(:,t) = sum(band_pop_mat(:,(t-103):(t-52)),2);
end

pop = zeros(size(matp,1),1);
for r = 1:length(pop)
    pop(r) = band_pop_sum_year(matp(r,3),matp(r,4));
end
pop_past = zeros(size(matp,1),1);
for r = 1:length(pop_past)
    pop_past(r) = band_pop_sum_past2_year(matp(r,3),matp(r,4));
end
pop_ratio = pop./pop_past;
pop_ratio(pop_ratio<0) = 1;
pop_ratio(isnan(pop_ratio)) = 0;           % if both are 0 assigned 1
pop_ratio(isinf(pop_ratio)) = 1;           % replace with highest valued observed
...
% beta_0 = b
beta_0 = [-7.7250    0.0443    0.3302    0.0686   0.01    0.01    0.01    0.001   0.0341    2.1842   0.7598]
clearvars b

[b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_past_year_ratio(X, trend_hat, pop_ratio, dummy_agg_SI, None0s_X_N, S, y, beta_0);
