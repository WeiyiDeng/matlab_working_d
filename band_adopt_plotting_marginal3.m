...
% % try hist plot with vertical lines
% hist(uni_innov_m,50)
% hist(uni_innov_f,50)
% hist(uni_explor_m,50)
% hist(uni_explor_f,50)
% text(-1,120000,'16%')
% text(2,120000,'84%')
...
% q_eval = 0:0.01:0.99;
q_eval = 0.01:0.01:1;
innov_m_q = quantile(uni_innov_m,q_eval);
innov_f_q = quantile(uni_innov_f,q_eval);
explor_m_q = quantile(uni_explor_m,q_eval);
explor_f_q = quantile(uni_explor_f,q_eval);
...
innov_m_plug = median(uni_innov_m);
innov_f_plug = median(uni_innov_f);
explor_m_plug = median(uni_explor_m);
explor_f_plug = median(uni_explor_f);

prob = zeros(length(q_eval),2);
for d = 1:length(WD_plug)
...
%         explor_f_plug = explor_f_q(k);
        ...
        week_IV = 100*gampdf(WD_plug(d),exp(b(2)),exp(b(3)));
        week_IV(WD_plug(d)<1)=0;
...
hist(ax1,uni_innov_m,50)
% hist(ax1,uni_innov_f,50)                                          %<- to change
% hist(ax1,uni_explor_m,50)
% hist(ax1,uni_explor_f,50)

hold on
% plot(innov_m_q,prob)

up_bound = 0.015;                                                             %<- to change
low_bound = -0.005;                                                           
x = linspace(min(uni_innov_m),max(uni_innov_m),100);                          %<- to change *2 !!
y = low_bound.*ones(size(x));
plot(ax2,x,y,'k')

delta_prob = prob(:,1)-prob(:,2);

% plot(ax2,innov_f_q,prob,'r','linewidth',0.001)                               %<- to change
plot(ax2,innov_m_q,delta_prob,'-r.','linewidth',0.001)
% plot(ax2,innov_m_q,prob,'r.')
hold off

x_point = quantile(uni_innov_m,[0.16 0.84]);                           %<- to change
...
line([x_point(1) x_point(1)],[-0.01 0.015], 'Color', 'r','Linestyle','--')
line([x_point(2) x_point(2)],[-0.01 0.015], 'Color', 'r','Linestyle','--')
text(x_point(1)+0.1,0,'16%')
text(x_point(2)+0.1,0,'84%')

ylabel(ax1,'num of users')
ylabel(ax2,'likelihood change')
