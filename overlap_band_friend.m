% overlapping weeks between friends and bands
...
prob_adoption_yearj = diffusion_year./repmat(sum_adoptions_yearj,T,1);    % PAjt 
...
overlap_friend_band = zeros(200000,1);
% largeNum_p = 68000000
% % largeNum_p = 600000
% 
...
%             abs_week_diff(ind:ind+interval-1) = abs((pre_start:pre_end) - adoptmij_time);
            overlap_friend_band(ind) = interval;            
            ind = ind+1;
        else
        end
    end
...
overlap_friend_band = overlap_friend_band(1:(ind-1));

save('overlap_friend_band.mat','overlap_friend_band');
