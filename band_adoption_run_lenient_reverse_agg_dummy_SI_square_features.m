...
A = [pop topics_count tracks_count band_age];
corrcoef(A)
...
% beta_0 = b
beta_0 = [-7.7250    0.0443    0.3302    0.0686   0.01    0.01    0.01   0.01...
    0.01     0.001     0.001    0.001     0.001      0.001      0.001     0.001      0.001...
    0.0341    2.1842   0.7598]
clearvars b

% pop = topics_count;              % topics

[b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_trend_reverse_agg_dummy_SI_square_features(X, trend_hat, pop, topics_count, band_age, tracks_count, dummy_agg_SI, None0s_X_N, S, y, beta_0);

save('b_agg_dummy_topics.mat','b') ;
save('standard_error_agg_dummy_topics.mat','standard_error') ;
save('t_stat_agg_dummy_topics.mat','t_stat') ;
save('exit_flag_agg_dummy_topics.mat','exit_flag') ;
save('hessian_topics.mat','hessian') ;
