...
adopt_strict_all = csvread('adopt_strict_all_users_2588.csv');
adopt_strict_mat = sparse(adopt_strict_all(:,2), adopt_strict_all(:,1), adopt_strict_all(:,3), T, J);
% sum(sum(adopt_strict_mat))                    % 989070 adoptions in total
...
sum_adopt_strict_j = sum(adopt_strict_mat,1);
...
prob_strict_adoption_j = adopt_strict_mat./repmat(sum_adopt_strict_j,T,1);
    ...
    for j = 1:J
        ...
        if adoptfij_time~=0 && adoptmij_time~=0
            if adoptfij_time > timesplit(friend_id,3) && adoptmij_time > timesplit(member_id,3)
                ...
                prob_adopt_week(ind:ind+interval-1) = prob_strict_adoption_j(pre_start:pre_end,j);
...
for i = 1:numc-1
    ...
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\matp_%d.mat',i), 'C_matp');
    clearvars C_matp
end
if rem(I,numc)>0
    C_matp = matp(i*lenc+1:end,:);
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\matp_%d.mat',numc), 'C_matp');
else
end
...
for i = 1:numc-1
    ...
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\innov_contin_%d.mat',i), 'C_innov');
    clearvars C_innov
end
if rem(I,numc)>0
    C_innov = innov_contin(i*lenc+1:end,:);
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\innov_contin_%d.mat',numc), 'C_innov');
else
end
...
for i = 1:numc-1
    ...
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\explor_contin_%d.mat',i), 'C_explor');
    clearvars C_explor
end
if rem(I,numc)>0
    C_explor = explor_contin(i*lenc+1:end,:);
    save(sprintf('D:\\Users\\MATLAB_data\\matfolder\\explor_contin_%d.mat',numc), 'C_explor');
else
end
