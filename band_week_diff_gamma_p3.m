...
% clearvars matp
% 
% week_IV = 100*gampdf(IVs(:,2),0.9649,27.4735);              % w: NOTICE new lines here for fixed gamma parameters
% week_IV(IVs(:,2)<1)=0; 
% 
% rb = -10;
% ev_points = [];
...
%             const = -5.1854;
%             bs = [0.2615 0.1641 ev_points(d) ev_points(k)]';
%             FV = [IVs(:,1) week_IV innov_IV  innov_WD_multip explor_IV explor_WD_multip]*bs;
%             
%             exp_util = exp(-(const+FV));         % this is now the utility of the external good
%             prob=1./(1+exp_util);                % this is still the probability of choosing the product
...
for i = 1:length(b_k)
    for j = 1:length(b_theta)
        for d = 1:length(b_wd)
            ...
            b = [-5.0354 b_k(i) b_theta(j) 0.3250 b_wd(d)];
