...
high_pop = med_pop + std(pop)
low_pop = med_pop - std(pop)
% med_pop = median(pop);
...
for i = 1:length(test)
    for j = 1:length(pop_test)
        FV = [mean(trend_hat)  test(i)  test(i).^2   pop_test(j)   test(i).*pop_test(j)      test(i).^2.*pop_test(j)    mean(IV_N_S)]*b_basic;
        exp_util = exp(-(const+FV));
        prob=1./(1+exp_util);
        store_p(i,j) = prob;
%         store_u(i,j) = [test(i)  test(i).^2   pop_test(j)   test(i).*pop_test(j)      test(i).^2.*pop_test(j)]*b_basic(1:5);
        store_u(i,j) = [test(i)  test(i).^2   pop_test(j)   test(i).*pop_test(j)      test(i).^2.*pop_test(j)]*b_basic(3:7);
    end
end
store_p
store_u
...
med_pop = mean(pop)
% med_pop = mean(pop(pop>0))
high_pop = med_pop + 1*std(pop)
low_pop = med_pop - 1*std(pop)
...
for i = 1:length(test)
    FV = [mean(trend_hat)  test(i)  test(i).^2   high_pop   test(i).*high_pop      test(i).^2.*high_pop    mean(IV_N_S)]*b_basic;
    exp_util = exp(-(const+FV));
    prob=1./(1+exp_util);
    store(i) = prob;
    store_u(i) = [test(i)  test(i).^2   high_pop   test(i).*high_pop      test(i).^2.*high_pop]*b_basic(2:6);
end
...
store_u = zeros(size(test));
for i = 1:length(test)
    FV = [mean(trend_hat)  test(i)  test(i).^2   med_pop   test(i).*med_pop      test(i).^2.*med_pop    mean(IV_N_S)]*b_basic;
    exp_util = exp(-(const+FV));
    prob=1./(1+exp_util);
    store(i) = prob;
    store_u(i) = [test(i)  test(i).^2   med_pop   test(i).*med_pop      test(i).^2.*med_pop]*b_basic(2:6);
end
plot(test,store,'-','color','k')
% plot(test,store_u,'-','color','k')
% legend('low pop','Location','northwest')
...
store_u = zeros(size(test));
for i = 1:length(test)
    FV = [mean(trend_hat)  test(i)  test(i).^2   low_pop   test(i).*low_pop      test(i).^2.*low_pop    mean(IV_N_S)]*b_basic;
    exp_util = exp(-(const+FV));
    prob=1./(1+exp_util);
    store(i) = prob;
    store_u(i) = [test(i)  test(i).^2   low_pop   test(i).*low_pop      test(i).^2.*low_pop]*b_basic(2:6);
end
plot(test,store,'-.','color','k')
% plot(test,store_u,'-.','color','k')
legend({'high pop','median pop','low pop'},'Location','northwest')
hold off
xlabel('Social influence levels') 
ylabel('Hazard rate')
xticks([0 1 2])
...
for i = 1:length(test)
    ...
    prob=1./(1+exp_util);
    store(i) = prob;
    store_u(i) = [test(i)  test(i).^2]*b_basic(2:3);
end
figure
plot(test,store,'-','color','k')
% figure
