...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tag_pop = csvread('band_tag_pop_list.csv',1,0);
band_id_tag = csvread('band_id_list.csv',1,0);
tag_pop_vec = sparse(band_id_tag,1,tag_pop,6046,1);
pop = zeros(size(matp,1),1);
for r = 1:length(pop)
    pop(r) = tag_pop_vec(matp(r,3));
end
...
[b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_tag_pop(X, trend_hat, pop, dummy_agg_SI, None0s_X_N, S, y, beta_0);

save('b_tag_pop.mat','b') ;
save('standard_error_tag_pop.mat','standard_error') ;
save('t_stat_tag_pop.mat','t_stat') ;
save('exit_flag_tag_pop.mat','exit_flag') ;
save('hessian_tag_pop.mat','hessian') ;
