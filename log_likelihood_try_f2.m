function [LL, g, H] = log_likelihood_try_f2(b,DV,IV,I,P)

b_basics = b(1:5);

exp_util = exp(-[IV(:,1:3) gampdf(IV(:,4),1,exp(b(7))) normpdf(IV(:,5),0,exp(b(6)))]*b_basics');         % this is now the utility of the external good
...
g(4) = gampdf(IV(:,4),1,exp(b(7)))'*(prob-DV(:,1));
...
gamma_derivative_beta = gamma_derivative_wrt_beta(IV(:,4), 1, b(7)); 
