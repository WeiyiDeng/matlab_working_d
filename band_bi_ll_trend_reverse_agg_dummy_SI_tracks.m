...
function [LL, gr, H] = band_bi_ll_trend_reverse_agg_dummy_SI_tracks(b,IVs,trend_hat, band_age, topics_count, tracks_count, dummy_agg_SI, None0s_X_N, S, choice_dv)
global dummies se
...
val_pdf = 100*normpdf(None0s_X_N(:,3),0,b(12));
X_N = sparse(None0s_X_N(:,1),None0s_X_N(:,2),val_pdf,17617085,6222);
IV_N_S = X_N*S.^exp(b(13));
...
FV = [IVs(:,1)  trend_hat  week_IV  band_age  topics_count   tracks_count...
    band_age.*week_IV   topics_count.*week_IV    tracks_count.*week_IV    IV_N_S]*b_basic;          % with both trend and BP as controls
