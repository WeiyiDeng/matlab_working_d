function [LL, gr, H] = band_bi_ll_d(b, dummies)
...
% D = zeros(I,1);
% % DELETED dummies = dummies.';          % w: did this step in the previous function (not to do the inverse in every call)        
...
for i = 1:numc
    D((i-1)*lenc+1:i*lenc) = dummies((i-1)*lenc+1:i*lenc,:)*bs_d;
end
if rem(I,numc)>0
    D(i*lenc:end) = dummies(i*lenc:end,:)*bs_d;
else
end
