...
disp('run strict adoption with all member adopted bands with no trend data deleted and log(y_hat) as predicted trends and baseline probability both as controls') 
...
load('matp_trend_rm_all_member_strict.mat');
load('explor_contin_trend_rm_all_member_strict.mat');
load('innov_contin_trend_rm_all_member_strict.mat');       % w: inconsistent rows????????????
...
load('newbp_store_rm_all_member_strict.mat')       % variable name baseline_prob_store
