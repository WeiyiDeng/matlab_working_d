...
function [LL, gr, H] = band_bi_ll_constant(b,IVs,choice_dv)
global dummies se
...
const = b(1).*ones(length(choice_dv),1);
...
% % week_IV = 100*exppdf(IVs(:,2),b(2));
% week_IV(IVs(:,2)<1)=0;                                   % BEWARE THIS ! need to turn this on
