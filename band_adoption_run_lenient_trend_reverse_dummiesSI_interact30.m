...
dummy_prep = dummies_prep(:,1) | dummies_prep(:,2) | dummies_prep(:,3);         % 30
...
%         intercept baseline_prob gtrend(5) new_week_diff  band_age  topics_count  Age*new_week_diff     Diversity*new_week_diff(10)     Diversity*Age    Diversity*Age*new_week_diff     week_diff_coef_n  normal_sd   similarity_coef
beta_0 = [-7.0387    0.0368        0.3433      0.3         -0.0034      0.0052      0.1       0.1      0.1     0.1    0.0337    2.0827    1.9961]
clearvars b

[b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_trend_reverse_dummiesSI_interact(X, trend_hat, band_age, topics_count, dummy_prep, None0s_X_N, S, y, beta_0);

save('b_dummy30.mat','b') ;
save('standard_error_dummy30.mat','standard_error') ;
save('t_stat_dummy30.mat','t_stat') ;
save('exit_flag_dummy30.mat','exit_flag') ;
