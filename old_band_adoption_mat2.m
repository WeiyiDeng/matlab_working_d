...
EAcat = csvread('EAdummy.csv');          % dummy created from excel using EAcats.csv
...
member_a = zeros(600000,1);
friend_a = zeros(600000,1);
band_a = zeros(600000,1);
% timeobs = [];
DV1 = zeros(600000,1);
...
for i = 1:size(friendlist,1)                                         % i here  is the row num of friendlist
    ...
    for j = 1:J
        ...
        if isempty(row_adoptfij)==0 && adoptfij_time < timesplit(id_friend,3)      % remove obs of band j if it was adopted by i in period 1
            ...
            prep_start = find(product, 1);
            if isempty(prep_start)==0
                obs_start(i,j) = prep_start;
            else
            end
            %             if sum(product)>0
            if obs_start(i,j)>0
...
member_t = zeros(800000,1);
friend_t = zeros(800000,1);
band_t = zeros(800000,1);
timeobs = zeros(800000,1);
DV2 = zeros(800000,1);
friend_adopted = zeros(800000,1);
...
%     friend_adopted = [friend_adopted; zeros(sum(product),1)];
    member_t(ind:ind+sum(product)) = member_id;
    friend_t(ind:ind+sum(product)) = friend_id;
    band_t(ind:ind+sum(product)) = band_id;
    timeobs(ind:ind+sum(product)) = find(product)';
%     DV2 = [DV2; zeros(sum(product),1)];
%     friend_adopted = [friend_adopted; zeros(sum(product),1)];
...
bandpopular_a = zeros(length(band_a),1);
for i = 1:size(friendlist,1)                % i here is the row num of friendlist
    for j = 1:J
        if obs_start(i,j) > 0
