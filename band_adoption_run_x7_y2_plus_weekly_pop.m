...
pop_y = zeros(size(matp,1),1);
for r = 1:length(pop_y)
    pop_y(r) = band_pop_sum_year(matp(r,3),matp(r,4));
end

pop_w = zeros(size(matp,1),1);
for r = 1:length(pop_w)
    pop_w(r) = band_pop_mat(matp(r,3),matp(r,4));
end
...
% beta_0 = b
beta_0 = [-7.7250    0.0443    0.3302    0.0686   0.01    0.01    0.01    0.001   0.01    0.01    0.001   0.0341    2.1842   0.7598]
clearvars b

[b, hessian, grad, standard_error, covariance_matrix, t_stat, exit_flag, output] = band_runbi_ll_y2_plus_weekly_pop(X, trend_hat, pop_y, pop_w, dummy_agg_SI, None0s_X_N, S, y, beta_0);

save('b_y2_plus_weekly_pop.mat','b') ;
save('standard_error_y2_plus_weekly_pop.mat','standard_error') ;
save('t_stat_y2_plus_weekly_pop.mat','t_stat') ;
save('exit_flag_y2_plus_weekly_pop.mat','exit_flag') ;
save('hessian_y2_plus_weekly_pop.mat','hessian') ;
