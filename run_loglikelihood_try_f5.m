...
% opts = optimoptions(@fminunc,'Algorithm','trust-region','MaxIterations',1500, 'Display','iter')     % 'trust-region' or 'quasi-newton'
% problem = createOptimProblem('fminunc','objective',@(x) log_likelihood_try_f5(x,DV,IV),'x0',b0,'options',opts);
% ms = MultiStart;
% [b,fval,exitflag,output, solutions] = run(ms,problem,20)

opts = optimoptions(@fmincon,'Algorithm','interior-point','MaxIterations',1500, 'Display','iter')     % 'trust-region' or 'quasi-newton'
problem = createOptimProblem('fmincon','objective',@(x) log_likelihood_try_f5(x,DV,IV),'x0',b0,'options',opts);
gs = GlobalSearch;
