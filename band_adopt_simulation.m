...
% rep_times = 10;
rep_times = 1;
NM_set = repmat(20,rep_times,1);
% NM_set = [NM_set; repmat(40,rep_times,1)];
% NM_set = [NM_set; repmat(60,rep_times,1)];
% NM_set = [NM_set; repmat(80,rep_times,1)];
% NM_set = [NM_set; repmat(100,rep_times,1)];
...
beta_P2 = -0.3;
se_P2 = 3;
beta_A = 3;                               % social influence parameter
...
metrics_out_cell = cell(NM,1);
metrics_out_avg = zeros(NM,12);
for m = 1:NM
    m_metrics = metrics_store_cell{m};
...
clearvars -EXCEPT NM_set metrics_out_avg_set average_member_adopts d alpha_R se_R beta_C se_C beta_P1 se_P1 beta_P2 se_P2 beta_A

end

figure
subplot(3,2,1)
scatter(NM_set, metrics_out_avg_set(:,1))
title('within4Weeks_f')
subplot(3,2,2)
scatter(NM_set, metrics_out_avg_set(:,2))
% title('random4Weeks_f')
title('fixed4Weeks_f')
subplot(3,2,3)
scatter(NM_set, metrics_out_avg_set(:,3))
title('within4Weeks_n')
subplot(3,2,4)
scatter(NM_set, metrics_out_avg_set(:,4))
% title('random4Weeks_n')
title('fixed4Weeks_n')
subplot(3,2,5)
scatter(NM_set, metrics_out_avg_set(:,5))
title('# of friends')
subplot(3,2,6)
scatter(NM_set, metrics_out_avg_set(:,6))
title('# of neighbours')

figure
subplot(3,2,1)
scatter(NM_set, metrics_out_avg_set(:,7))
title('A_f(TI)/N_f')
subplot(3,2,2)
scatter(NM_set, metrics_out_avg_set(:,8))
% title('A_f(R)/N_f')
title('A_f(Fix)/N_f')
subplot(3,2,3)
scatter(NM_set, metrics_out_avg_set(:,9))
title('A_n(TI)/N_n')
subplot(3,2,4)
scatter(NM_set, metrics_out_avg_set(:,10))
% title('A_n(R)/N_n')
title('A_n(Fix)/N_n')
subplot(3,2,5)
scatter(NM_set, metrics_out_avg_set(:,11))
% title('A_f(TI)/N_f-A_f(R)/N_f')
title('A_f(TI)/N_f-A_f(Fix)/N_f')
subplot(3,2,6)
scatter(NM_set, metrics_out_avg_set(:,12))
% title('A_n(TI)/N_n-A_n(R)/N_n')
title('A_n(TI)/N_n-A_n(Fix)/N_n')

figure
scatter(NM_set, average_member_adopts)
