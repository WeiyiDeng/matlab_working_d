...
function [LL, gr, H] = band_bi_ll_paper2_innov_IiIk3(b,IVs,trend_hat, pop, dummy_agg_SI, mat_sparse_8088, m_innov, f_innov, cosine_similarity_scores, None0s_X_N, S, choice_dv)
global dummies se
...
val_pdf = 100*normpdf(None0s_X_N(:,3),0,b(7));
X_N = sparse(None0s_X_N(:,1),None0s_X_N(:,2),val_pdf,17617085,6222);
IV_N_S = X_N*S.^exp(b(8));
...
FV = [Aijkt*Sik_power    Aijkt*(m_innov.*Sik_power)    Aijkt*(f_innov.*Sik_power)    Aijkt*(f_innov.*m_innov.*Sik_power)     IV_N_S]*b_basic;
