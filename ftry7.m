% w: significance of explor member and friend ?
%% coefficients
clear all
...
LB = [-5.0951    0.19983    2.0459    0.3222    0.0627];
LB = [LB     -0.0102    0.0667   -0.1275    0.0555   -0.0245   -0.0836    -0.0396    0.0001];  
LB = [LB      0.0181   -0.0185    0.0131   -0.0125   -0.0214    -0.0016   -0.0234    -0.0011];
LB = [LB     -0.0160   -0.0046    -0.0128    -0.0021    0.0117    0.0008    -0.0029   -0.003]

UB = [-5.0951    0.19983    2.0459    0.3222    0.0627];
UB = [UB     -0.0102    0.0667   -0.1275    0.0555   0.0235   -0.0468    -0.0054    0.0102];  
UB = [UB      0.0181   -0.0185    0.0131   -0.0125   0.0038    0.02   -0.0056    0.0045];
UB = [UB     -0.0160   -0.0046    0.0182    0.0031    0.0117    0.0008    0   0]
...
prob = zeros(size(nbs,1),length(explor_intv_m),length(explor_intv_f));      % To change!!
...
for r = 1:size(nbs,1)
    ...
    for q = 1:length(explor_intv_m)                                                 % To change!!
        for g = 1:length(explor_intv_f)                                             % To change!!
            %     innov_m_plug = -0.2244;
            %     innov_f_plug = -0.1805;
...
prob = zeros(length(WD_plug),length(explor_intv_m),length(explor_intv_f));      % To change!!
for q = 1:length(explor_intv_m)                                                 % To change!!
    for g = 1:length(explor_intv_f)                                             % To change!!
        %     innov_m_plug = -0.2244; 
        %     innov_f_plug = -0.1805;
...
sub_prob = prob(2,:,:)-prob(1,:,:);
% surf(squeeze(prob(2,:,:)))
h = surf(explor_intv_m, explor_intv_f, squeeze(sub_prob));
% h = surf(explor_intv_m, explor_intv_f, squeeze(prob(2,:,:)));
...
zlabel('sig prob');
hold on
contour(explor_intv_m, explor_intv_f, sig_prob.*0.001)
