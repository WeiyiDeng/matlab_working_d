...
log_y = log(paper3_DV+1);
md2 = fitlm(X,log_y)

md3 = fitlm(X_std,y)

X_const = [ones(size(paper3_DV_lag)) X];
b_est = inv(X_const'*X_const)*X_const'*log_y

modelresults = mdl.Coefficients.Estimate
modelresults_std = md3.Coefficients.Estimate

RMSE1 = mean(sum((X_const*modelresults-paper3_DV).^2))
RMSE2 = mean(sum((exp(X_const*b_est)-paper3_DV).^2))
RMSE3 = mean(sum(([ones(size(paper3_DV_lag)) X_std]*modelresults_std-paper3_DV).^2))
...
sth = [ones(size(paper3_DV_lag)) X]*b_est;

member_id = m_t_agg(1,1);
m_end = [];
m_start = 1;
m_list = [];
for i = 2:size(m_t_agg,1)
    prev_m = member_id;
    member_id = m_t_agg(i,1);
    if prev_m ~=member_id
        m_end = [m_end; i-1];
        m_start = [m_start; i];
...
for t = 1:10
    ...
    for i = 1:length(m_list)
        lag_y_temp = y_pred(m_start(i):m_end(i)-t);
        m_t_agg_temp = m_t_agg(m_start(i)+t:m_end(i),:);
        m_t_agg_rec = [m_t_agg_rec; m_t_agg_temp];
        lag_y_rec = [lag_y_rec; lag_y_temp];
    end
    SimuX_rec = [ones(size(lag_y_rec,1),1) zeros(size(lag_y_rec,1),1) m_t_agg_rec(:,4) lag_y_rec zeros(size(lag_y_rec,1),1) m_t_agg_rec(:,4).*lag_y_rec];
