...
axis([0,130,0,240])
daspect([1,1,1]) 
k=1;d=1;df=0;r(k)=0;x(k)=0;y(k)=0;i=1;flag_new_fire=0;s=0;
...
for d =1:80
      ...
      if d == df+8
          ...
          x_new=unifrnd(0,130,1); %在0-130产生一个随机数赋给x
          y_new=unifrnd(0,240,1); %在0-240产生一个随机数赋给y
   ...
   rectangle('Position',[x(k)-r(k),y(k)-r(k),2*r(k),2*r(k)],'Curvature',[1,1],...
     'FaceColor','r')            % w: 用rectangle画圆
       ...
       saveas(gcf, 'output', 'bmp') %Write current figure to windows bitmap file
       image=imread('output', 'bmp');%读入output图
       level= graythresh(image); %自动确定阈值水平
       a=im2bw(image,level);     %转化为黑白二值
       sum=0;
       threshhold=level;
       for p=1:size(a,1) %图像的行数
       for t=1:size(a,2) %图像的列数
          if a(p,t)>=threshhold %大于阈值，是白色亮的部分
          sum=sum+1;
          end
       end
       end
       if sum/(size(a,1)*size(a,2))<=0.9 %若没烧面积<90%则中断
           print d 
           print sum/(size(a,1)*size(a,2))
           break
       end
     end     
       pause(0.01) %延迟一定的时间便于观察图形变化
 end